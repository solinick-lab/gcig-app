"""Pure quantile-regression CPI nowcaster.

The baseline `nowcast.py` averages a Ridge point forecast with a single
quantile-median GBR, then fakes an 80% interval by adding +/- 1.2816 *
residual_std around the mean. Two assumptions creak there: (1) Gaussian
error tails on a series that very obviously has heavy/asymmetric tails
around energy shocks, (2) the point forecast drifts toward the
conditional MEAN rather than the more robust conditional MEDIAN.

This module replaces the ensemble with three independently-fit
GradientBoostingRegressors using pinball ('quantile') loss at
alpha = 0.1, 0.5, 0.9. The conditional median (q=0.5) becomes the
point forecast; the 0.1 / 0.9 quantiles form an honest 80% interval
without any normality assumption. Quantile crossing (lo > median > hi
violation) is handled post-hoc by sorting the triple — a known issue
when fitting separate quantile regressors on small samples.

Reuses the supervised-feature builder from `nowcast` so feature
engineering, lag conventions, and as-of timing stay identical to the
baseline. Only the head model changes.

Sanity: MoM is clipped to [-1.5, 2.5] before the YoY chain to avoid
absurd outliers when the GBR extrapolates out of distribution.

Each historical cut is wrapped in try/except — one bad fit doesn't
kill the 24-month walk-forward backtest.
"""
...
def fit_quantile_nowcast_model(
    panel: pd.DataFrame,
    daily_frame: dict[str, pd.Series],
    as_of_day: int = DEFAULT_AS_OF_DAY,
) -> QuantileNowcastModel:
    """Fit three quantile-loss GBRs on the supervised (X, y) panel."""
...
def run_quantile_nowcast(as_of_day: int = DEFAULT_AS_OF_DAY) -> QuantileNowcastResult:
    """Pull live panels, fit quantile model, produce a current-month forecast."""
    panel = fetch_panel()
    daily_panel = get_daily_panel()
    daily_frame = build_daily_frame(daily_panel)

    model = fit_quantile_nowcast_model(panel, daily_frame, as_of_day=as_of_day)
...
def backtest_quantile_nowcast(
    panel: pd.DataFrame,
    daily_frame: dict[str, pd.Series],
    window_months: int = 24,
    as_of_day: int = DEFAULT_AS_OF_DAY,
) -> dict:
    """Walk-forward backtest of the pure quantile nowcaster.

    For each historical cut t in the trailing `window_months`:
      - train three quantile GBRs on data strictly BEFORE t
