"""Split-conformal calibration on top of the quantile nowcaster.

The pure quantile model (`nowcast_quantile.py`) achieves a 0.1451 RMSE
YoY with 95.8% within +/-0.25pp on the trailing 24-month walk-forward.
That's strong, but two known weaknesses remain when fitting q={0.1,
0.5, 0.9} GBRs on small samples:

  1. Median bias. The pinball loss is robust, but with ~10-15 years of
     monthly data the in-sample median can drift away from the true
     conditional median, leaving a small but persistent signed bias.
  2. Miscalibrated intervals. Nominally 80% bands from q0.1/q0.9 rarely
     achieve 80% empirical coverage on out-of-sample data — they tend
     to be too tight on the heavy-tailed CPI process.

Split conformal prediction fixes both, distribution-free, with a
finite-sample coverage guarantee. The recipe:

  - Chronologically split the supervised (X, y) into a proper-train
    block (first 80%) and a calibration block (last 20%). The temporal
    split matters: shuffling would leak future structure backward.
  - Fit q={0.1, 0.5, 0.9} GBRs on the proper-train block ONLY.
  - On the calibration block:
      * signed residuals r_i = y_i - yhat_50_i           (median bias)
      * nonconformity   s_i = max(yhat_lo_i - y_i,
                                  y_i - yhat_hi_i, 0)    (interval gap)
  - Bias correction:    delta = mean(r_i)
        adjusted_med = yhat_50 + delta
  - Interval correction: q_hat = ceil((n+1)*0.8)/n quantile of {s_i}
        adjusted_lo = yhat_lo - q_hat
        adjusted_hi = yhat_hi + q_hat
  - REFIT all three quantile GBRs on the FULL training set before
    inference (calibration is done; we want maximum data efficiency
    for the deployed model — only the calibration constants travel).

If the calibration set has fewer than 10 rows we skip conformal and
fall back to plain sorted quantile predictions, since q_hat at n<10
has too much variance to help.

Each cut is wrapped in try/except so a single failed fit doesn't poison
the backtest. Reuses `_build_supervised` and the MoM->YoY chain logic
from the existing modules — only the head model layer is augmented.
"""

from __future__ import annotations

import warnings
from dataclasses import dataclass

import numpy as np
import pandas as pd
from sklearn.ensemble import GradientBoostingRegressor

from .api_client import get_daily_panel
from .features import build_target
from .fred import TARGET, fetch_panel
from .nowcast import _as_of_for_month, _build_supervised, DEFAULT_AS_OF_DAY
from .nowcast_features import build_daily_frame, features_at


warnings.filterwarnings("ignore")


# --- constants -------------------------------------------------------

_QUANTILES = (0.1, 0.5, 0.9)
_MOM_LO_CLIP = -1.5
_MOM_HI_CLIP = 2.5
_RESID_FLOOR = 0.05  # minimum half-width on YoY interval to avoid collapse
_CAL_FRACTION = 0.20  # tail share held out for calibration
_MIN_CAL_ROWS = 10    # below this, fall back to plain quantile
_TARGET_COVERAGE = 0.80

_GBR_PARAMS = dict(
    n_estimators=400,
    max_depth=3,
    learning_rate=0.05,
    random_state=42,
)


@dataclass
class ConformalNowcastResult:
    as_of: pd.Timestamp
    target_month: str
    pred_mom: float
    pred_yoy: float
    lo80_yoy: float
    hi80_yoy: float
    days_observed: int


@dataclass
class ConformalNowcastModel:
    models: dict[float, GradientBoostingRegressor]
    feature_cols: list[str]
    as_of_day: int
    bias_shift: float       # mean signed residual on calibration set (delta)
    interval_pad: float     # q_hat from conformity scores
    used_conformal: bool    # False if we fell back to plain quantile

    def predict_one(self, x: pd.Series) -> tuple[float, float, float]:
        """Return (median_corrected, lo_corrected, hi_corrected) MoM.

        Sorts the raw triple to enforce monotonicity in case of quantile
        crossing, then applies the calibration constants. If conformal
        was skipped (small calibration set), bias_shift=0 and
        interval_pad=0 so this degenerates to plain sorted quantile.
        """
        x_aligned = x.reindex(self.feature_cols).fillna(0.0).values.reshape(1, -1)
        preds = []
        for q in _QUANTILES:
            preds.append(float(self.models[q].predict(x_aligned)[0]))
        triple = np.sort(np.array(preds, dtype=float))
        raw_lo, raw_med, raw_hi = float(triple[0]), float(triple[1]), float(triple[2])
        med = raw_med + self.bias_shift
        lo = raw_lo - self.interval_pad + self.bias_shift
        hi = raw_hi + self.interval_pad + self.bias_shift
        return med, lo, hi


def _fit_quantile_models(
    Xv: np.ndarray, yv: np.ndarray,
) -> dict[float, GradientBoostingRegressor]:
    """Fit one GBR per quantile in `_QUANTILES`."""
    models: dict[float, GradientBoostingRegressor] = {}
    for q in _QUANTILES:
        models[q] = GradientBoostingRegressor(
            loss="quantile", alpha=q, **_GBR_PARAMS,
        ).fit(Xv, yv)
    return models


def fit_conformal_nowcast_model(
    panel: pd.DataFrame,
    daily_frame: dict[str, pd.Series],
    as_of_day: int = DEFAULT_AS_OF_DAY,
) -> ConformalNowcastModel:
    """Fit quantile GBRs with split-conformal calibration on top.

    Steps:
      1. Build supervised (X, y) — same builder the baseline uses.
      2. Chronological 80/20 split into proper-train / calibration.
      3. Fit q={0.1, 0.5, 0.9} GBRs on proper-train ONLY.
      4. Score calibration: signed residuals (median) + nonconformity
         scores (interval).
      5. Compute bias_shift (mean signed resid) and interval_pad
         (q_hat = ceil((n+1)*0.8)/n quantile of nonconformity scores).
      6. REFIT the same three GBRs on the FULL (X, y) — the deployed
         model uses every row of training data; only the calibration
         constants from step 5 are kept.

    If calibration size is below `_MIN_CAL_ROWS`, fall back to plain
    quantile (no shift, no pad) — too few points to estimate q_hat
    reliably.
    """
    X, y = _build_supervised(panel, daily_frame, as_of_day=as_of_day)
    cols = list(X.columns)
    Xv = X.values
    yv = y.values
    n_total = len(yv)

    # Default: no calibration adjustments. Overwritten below if cal succeeds.
    bias_shift = 0.0
    interval_pad = 0.0
    used_conformal = False

    try:
        n_cal = int(np.floor(n_total * _CAL_FRACTION))
        n_train = n_total - n_cal
        if n_cal >= _MIN_CAL_ROWS and n_train >= _MIN_CAL_ROWS:
            X_train_v = Xv[:n_train]
            y_train_v = yv[:n_train]
            X_cal_v = Xv[n_train:]
            y_cal_v = yv[n_train:]

            cal_models = _fit_quantile_models(X_train_v, y_train_v)

            yhat_lo = cal_models[0.1].predict(X_cal_v)
            yhat_med = cal_models[0.5].predict(X_cal_v)
            yhat_hi = cal_models[0.9].predict(X_cal_v)

            # Enforce monotonicity row-wise before scoring (defensive).
            stacked = np.vstack([yhat_lo, yhat_med, yhat_hi])
            stacked = np.sort(stacked, axis=0)
            yhat_lo, yhat_med, yhat_hi = stacked[0], stacked[1], stacked[2]

            signed_resid = y_cal_v - yhat_med
            bias_shift = float(np.mean(signed_resid))

            # Nonconformity for symmetric two-sided coverage:
            #   s_i = max(yhat_lo - y, y - yhat_hi, 0)
            nonconf = np.maximum.reduce([
                yhat_lo - y_cal_v,
                y_cal_v - yhat_hi,
                np.zeros_like(y_cal_v),
            ])

            n = len(nonconf)
            # Standard split-conformal level: ceil((n+1)*alpha)/n quantile,
            # with alpha=target coverage. Clip to [0, 1] for safety.
            level = min(1.0, np.ceil((n + 1) * _TARGET_COVERAGE) / n)
            interval_pad = float(np.quantile(nonconf, level))
            if not np.isfinite(interval_pad) or interval_pad < 0:
                interval_pad = 0.0
            used_conformal = True
    except Exception:
        # Any failure during calibration -> plain quantile fallback.
        bias_shift = 0.0
        interval_pad = 0.0
        used_conformal = False

    # Final inference model: refit on the FULL training data so we don't
    # waste the 20% calibration block once we've extracted its constants.
    final_models = _fit_quantile_models(Xv, yv)

    return ConformalNowcastModel(
        models=final_models,
        feature_cols=cols,
        as_of_day=as_of_day,
        bias_shift=bias_shift,
        interval_pad=interval_pad,
        used_conformal=used_conformal,
    )


def _build_inference_features(
    train_panel: pd.DataFrame,
    daily_frame: dict[str, pd.Series],
    target_month_end: pd.Timestamp,
    as_of_day: int,
) -> tuple[dict[str, float], pd.Timestamp]:
    """Feature row for the cut, simulating as-of `as_of_day` of target month."""
    m_start = target_month_end + pd.offsets.MonthBegin(-1)
    as_of = _as_of_for_month(m_start, as_of_day)
    feats = features_at(daily_frame, as_of)
    train_y = build_target(train_panel).dropna()
    cpi_train = train_panel[TARGET.fred_id].dropna()
    feats["cpi_mom_lag1"] = float(train_y.iloc[-1])
    feats["cpi_mom_lag2"] = float(train_y.iloc[-2])
    feats["cpi_yoy_lag1"] = float(
        (cpi_train.iloc[-1] / cpi_train.iloc[-13] - 1.0) * 100.0
    )
    feats["month_sin"] = float(np.sin(2 * np.pi * target_month_end.month / 12.0))
    feats["month_cos"] = float(np.cos(2 * np.pi * target_month_end.month / 12.0))
    return feats, as_of


def _mom_to_yoy(
    pred_mom: float,
    last_cpi: float,
    target_month_end: pd.Timestamp,
    cpi: pd.Series,
) -> float:
    """Convert predicted MoM log-% to YoY % using the standard chain."""
    predicted_cpi = last_cpi * float(np.exp(pred_mom / 100.0))
    denom_idx = target_month_end - pd.DateOffset(years=1)
    denom_idx = pd.Timestamp(denom_idx) + pd.offsets.MonthEnd(0)
    try:
        denom = float(cpi.loc[denom_idx])
    except KeyError:
        denom = float(cpi.asof(denom_idx))
    return (predicted_cpi / denom - 1.0) * 100.0


def run_conformal_nowcast(as_of_day: int = DEFAULT_AS_OF_DAY) -> ConformalNowcastResult:
    """Pull live panels, fit conformal-quantile model, produce current-month forecast."""
    panel = fetch_panel()
    daily_panel = get_daily_panel()
    daily_frame = build_daily_frame(daily_panel)

    model = fit_conformal_nowcast_model(panel, daily_frame, as_of_day=as_of_day)

    today = pd.Timestamp.utcnow().tz_localize(None).normalize()
    cpi = panel[TARGET.fred_id].dropna()
    last_released_month_end = cpi.index[-1]
    target_month_end = (last_released_month_end + pd.offsets.MonthBegin(1)) + pd.offsets.MonthEnd(0)
    target_month_start = target_month_end + pd.offsets.MonthBegin(-1)

    as_of = min(today, target_month_end)
    if today < target_month_start:
        as_of = _as_of_for_month(target_month_start, as_of_day)

    feats = features_at(daily_frame, as_of)
    y_mom = build_target(panel).dropna()
    feats["cpi_mom_lag1"] = float(y_mom.iloc[-1])
    feats["cpi_mom_lag2"] = float(y_mom.iloc[-2])
    feats["cpi_yoy_lag1"] = float((cpi.iloc[-1] / cpi.iloc[-13] - 1.0) * 100.0)
    feats["month_sin"] = float(np.sin(2 * np.pi * target_month_end.month / 12.0))
    feats["month_cos"] = float(np.cos(2 * np.pi * target_month_end.month / 12.0))

    pred_mom, lo_mom, hi_mom = model.predict_one(pd.Series(feats))
    pred_mom = float(np.clip(pred_mom, _MOM_LO_CLIP, _MOM_HI_CLIP))

    last_cpi = float(cpi.iloc[-1])
    pred_yoy = _mom_to_yoy(pred_mom, last_cpi, target_month_end, cpi)
    lo80_yoy = _mom_to_yoy(lo_mom, last_cpi, target_month_end, cpi)
    hi80_yoy = _mom_to_yoy(hi_mom, last_cpi, target_month_end, cpi)

    if (hi80_yoy - pred_yoy) < _RESID_FLOOR:
        hi80_yoy = pred_yoy + _RESID_FLOOR
    if (pred_yoy - lo80_yoy) < _RESID_FLOOR:
        lo80_yoy = pred_yoy - _RESID_FLOOR

    days_observed = sum(
        1 for s in daily_frame.values()
        if len(s.loc[(s.index >= target_month_start) & (s.index <= as_of)]) > 0
    )

    return ConformalNowcastResult(
        as_of=as_of,
        target_month=target_month_end.strftime("%Y-%m"),
        pred_mom=pred_mom,
        pred_yoy=pred_yoy,
        lo80_yoy=lo80_yoy,
        hi80_yoy=hi80_yoy,
        days_observed=days_observed,
    )


def backtest_conformal_nowcast(
    panel: pd.DataFrame,
    daily_frame: dict[str, pd.Series],
    window_months: int = 24,
    as_of_day: int = DEFAULT_AS_OF_DAY,
) -> dict:
    """Walk-forward backtest of the split-conformal quantile nowcaster.

    For each historical cut t in the trailing `window_months`:
      - take train_panel = panel[< t]
      - chronologically split into proper-train (80%) + calibration (20%)
      - fit q={0.1, 0.5, 0.9} GBRs on proper-train, score on calibration
      - compute bias_shift and interval_pad from calibration scores
      - REFIT GBRs on the full train_panel, apply the calibration
        constants to predict t at q=0.5 (corrected) and the interval
      - clip MoM to [-1.5, 2.5], chain to YoY against actual CPI[t-12]

    A single failed cut (insufficient history, fit failure, etc.) is
    skipped via try/except — it shouldn't poison the rest of the window.
    Falls back to plain quantile when calibration set has fewer than 10
    rows, which is rare given typical histories of ~120-150 months.

    Return shape mirrors `nowcast.backtest_nowcast` exactly.
    """
    cpi = panel[TARGET.fred_id].dropna()
    y_mom = build_target(panel).dropna()

    cuts = list(range(len(y_mom) - window_months, len(y_mom)))
    preds_mom: list[float] = []
    actuals_mom: list[float] = []
    preds_yoy: list[float] = []
    actuals_yoy: list[float] = []
    rows: list[dict] = []

    for ci in cuts:
        try:
            target_month_end = y_mom.index[ci]
            train_panel = panel.loc[panel.index < target_month_end]
            if len(train_panel) < 60:
                continue

            model = fit_conformal_nowcast_model(
                train_panel, daily_frame, as_of_day=as_of_day,
            )

            train_y = build_target(train_panel).dropna()
            if len(train_y) < 13:
                continue

            feats, as_of = _build_inference_features(
                train_panel, daily_frame, target_month_end, as_of_day,
            )

            pred_mom, _, _ = model.predict_one(pd.Series(feats))
            pred_mom = float(np.clip(pred_mom, _MOM_LO_CLIP, _MOM_HI_CLIP))
            actual_mom = float(y_mom.iloc[ci])

            last_cpi_train = float(train_panel[TARGET.fred_id].dropna().iloc[-1])
            pred_yoy = _mom_to_yoy(pred_mom, last_cpi_train, target_month_end, cpi)
            actual_cpi = float(cpi.loc[target_month_end])
            denom_idx = target_month_end - pd.DateOffset(years=1)
            denom_idx = pd.Timestamp(denom_idx) + pd.offsets.MonthEnd(0)
            try:
                denom = float(cpi.loc[denom_idx])
            except KeyError:
                denom = float(cpi.asof(denom_idx))
            actual_yoy = (actual_cpi / denom - 1.0) * 100.0

            preds_mom.append(pred_mom)
            actuals_mom.append(actual_mom)
            preds_yoy.append(pred_yoy)
            actuals_yoy.append(actual_yoy)
            rows.append({
                "target_month": target_month_end.strftime("%Y-%m"),
                "as_of": as_of.strftime("%Y-%m-%d"),
                "pred_mom": round(pred_mom, 4),
                "actual_mom": round(actual_mom, 4),
                "pred_yoy": round(pred_yoy, 3),
                "actual_yoy": round(actual_yoy, 3),
                "yoy_err": round(pred_yoy - actual_yoy, 3),
            })
        except Exception:
            # One bad cut shouldn't tank the whole backtest.
            continue

    if not preds_mom:
        return {"error": "no successful cuts"}

    pm = np.array(preds_mom); am = np.array(actuals_mom)
    py = np.array(preds_yoy); ay = np.array(actuals_yoy)
    yoy_err = np.abs(py - ay)
    return {
        "asOfDay": as_of_day,
        "windowMonths": window_months,
        "totalCuts": len(preds_mom),
        "rmseMom": float(np.sqrt(np.mean((pm - am) ** 2))),
        "rmseYoy": float(np.sqrt(np.mean((py - ay) ** 2))),
        "maeYoy": float(np.mean(yoy_err)),
        "hitWithin25bp": float((yoy_err <= 0.25).mean()) * 100,
        "hitWithin50bp": float((yoy_err <= 0.50).mean()) * 100,
        "rows": rows,
    }
