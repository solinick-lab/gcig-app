"""Quantile_rich + Cleveland Fed + Zillow ZORI + ZHVI nowcast.

Strategy: extend `nowcast_zillow.py` (which uses ZORI as a leading
indicator for shelter) by ADDING the Zillow Home Value Index (ZHVI). ZHVI
captures sale prices — leading indicator for housing wealth, mortgage
payments, and ultimately rents themselves.

Why ZHVI helps on top of ZORI:
  * ZHVI moves on home sale prices. When prices rise, mortgage payments
    rise (monthly housing cost up), housing wealth rises (consumption
    pressure), and landlord cap-rate math changes. All of these eventually
    feed CPI shelter.
  * Cap rate considerations: when home prices rise faster than rents,
    landlords push rents up to maintain yield. A high ZHVI/ZORI ratio
    today predicts rent acceleration tomorrow.
  * Home prices feed rents SLOWLY — typically 12-24 months. So we use
    longer lags (18 and 24) than ZORI's 12.

Features added on top of nowcast_zillow:
  * zhvi_yoy_lag18  — ZHVI YoY 18 months ago (mid-lead for rents)
  * zhvi_yoy_lag24  — ZHVI YoY 24 months ago (long-lead for rents)
  * zhvi_yoy_lag0   — most recent ZHVI YoY (housing momentum NOW)
  * zhvi_zori_ratio — current ZHVI/ZORI level ratio (cap-rate proxy)
  * zhvi_used_scrape — 1 if ZHVI present, 0 if missing/fallback

If the ZHVI block is unavailable (server payload missing it) we fall back
gracefully: the ZHVI features become NaN/0 and the model continues with
the ZORI + Cleveland feature set.

Each cut wrapped in try/except. MoM clipped to [-1.5, 2.5].

Public API:
  backtest_zhvi_nowcast(panel, daily_frame, window_months=24, as_of_day=20) -> dict
  run_zhvi_nowcast(as_of_day=20) -> ZhviNowcastResult
"""
...
def _zhvi_history_to_series(zori_payload: dict) -> pd.DataFrame | None:
    """Convert the `zhvi.history` block of the Zillow API response to a
    date-indexed DataFrame. Returns None if missing/malformed.

    Index = month-end timestamps. Columns: level, yoy, mom.
    """
    if not isinstance(zori_payload, dict):
        return None
    zhvi = zori_payload.get("zhvi")
    if not isinstance(zhvi, dict):
        return None
    hist = zhvi.get("history") or []
    if not isinstance(hist, list) or len(hist) < 25:
        # Need at least ~24 months of history so lag-24 is usable on the
        # most recent training cuts.
        return None
    rows: list[dict] = []
    for entry in hist:
...
def _zhvi_features_for_month(
    zhvi_df: pd.DataFrame | None,
    zori_df: pd.DataFrame | None,
    target_month_end: pd.Timestamp,
    is_zhvi_available: bool,
) -> dict[str, float]:
    """Build ZHVI features for one target month.

    Same publication-lag handling as the ZORI features: only data with
    month_end strictly < target_month_end is used.

    Returned features:
      zhvi_yoy_lag0   — most recent ZHVI YoY
      zhvi_yoy_lag18  — ZHVI YoY 18 months ago (mid-lead for rents)
      zhvi_yoy_lag24  — ZHVI YoY 24 months ago (long-lead for rents)
      zhvi_zori_ratio — ZHVI level / ZORI level at the same cutoff month
                        (cap-rate proxy; high = home prices outpacing rents
                         = future rent pressure)
      zhvi_mom_lag0   — most recent ZHVI MoM (price acceleration signal)
      zhvi_used_scrape — 1 if ZHVI present, 0 if missing
    """
    feats: dict[str, float] = {
        "zhvi_yoy_lag0": np.nan,
        "zhvi_yoy_lag18": np.nan,
        "zhvi_yoy_lag24": np.nan,
        "zhvi_zori_ratio": np.nan,
        "zhvi_mom_lag0": np.nan,
        "zhvi_used_scrape": 1.0 if is_zhvi_available else 0.0,
    }
    if zhvi_df is None or zhvi_df.empty:
        feats["zhvi_used_scrape"] = 0.0
        return feats

    # Only use ZHVI strictly before the target month.
    cutoff = target_month_end + pd.offsets.MonthEnd(-1)
    avail = zhvi_df.loc[zhvi_df.index <= cutoff]
    if avail.empty:
        return feats

    last = avail.iloc[-1]
    if np.isfinite(last.get("yoy", np.nan)):
        feats["zhvi_yoy_lag0"] = float(last["yoy"])
    if np.isfinite(last.get("mom", np.nan)):
        feats["zhvi_mom_lag0"] = float(last["mom"])

    if len(avail) >= 19:
        lag18 = avail.iloc[-19]
        if np.isfinite(lag18.get("yoy", np.nan)):
            feats["zhvi_yoy_lag18"] = float(lag18["yoy"])
    if len(avail) >= 25:
        lag24 = avail.iloc[-25]
        if np.isfinite(lag24.get("yoy", np.nan)):
            feats["zhvi_yoy_lag24"] = float(lag24["yoy"])

    # Cap-rate proxy: ratio of home value to annualized rent. We don't
    # know absolute rent (ZORI is a smoothed asking-rent INDEX, not a
    # dollar amount), but the *ratio of levels* normalized over time
    # still tracks the relative direction of cap rates. We use the same
    # cutoff month from both series.
    if zori_df is not None and not zori_df.empty:
        zori_avail = zori_df.loc[zori_df.index <= cutoff]
        if not zori_avail.empty:
            z_last = zori_avail.iloc[-1]
            if (
                np.isfinite(last.get("level", np.nan))
                and np.isfinite(z_last.get("level", np.nan))
                and float(z_last["level"]) > 0
            ):
                feats["zhvi_zori_ratio"] = float(last["level"]) / float(z_last["level"])
...
def _build_supervised_zhvi(
    ...
    """Quantile_rich + Cleveland + ZORI + ZHVI features per month."""
    ...
    for month_end in eligible_months:
        ...
        # Lagged CPI moments (same as nowcast_zillow / nowcast_clev).
        try:
        ...
        # ZORI features (lag-0/6/12 of YoY + lag-0 MoM) — already known
        # leading indicator for shelter.
        try:
            feats.update(
        ...
        # ZHVI features (lag-0/18/24 of YoY + ratio + lag-0 MoM).
        try:
            feats.update(
                _zhvi_features_for_month(
                    zhvi_df, zori_df, month_end, is_zhvi_available
                )
            )
        except Exception:
            pass
...
def backtest_zhvi_nowcast(
    panel: pd.DataFrame,
    daily_frame: dict[str, pd.Series],
    window_months: int = 24,
    as_of_day: int = DEFAULT_AS_OF_DAY,
) -> dict:
    """Walk-forward backtest: quantile_rich + Cleveland + ZORI + ZHVI.

    Calls each scrape ONCE up-front. ZHVI is read from the same
    `zillow-rent` payload (`payload.zhvi.history`) so we don't pay an
    extra round-trip.
    """
    cpi = panel[TARGET.fred_id].dropna()
    y_mom = build_target(panel).dropna()
    ...
    # ZHVI is a sub-block of the same payload. Wrap in try/except per spec
    # so a malformed `zhvi` field can't kill the whole backtest.
    try:
        zhvi_df = _zhvi_history_to_series(zori)
    except Exception:
        zhvi_df = None
    used_zhvi = zhvi_df is not None
    ...
    for ci in cuts:
        try:
            ...
            try:
                feats.update(
                    _zhvi_features_for_month(
                        zhvi_df, zori_df, target_month_end, used_zhvi
                    )
                )
            except Exception:
                pass
...
def run_zhvi_nowcast(as_of_day: int = DEFAULT_AS_OF_DAY) -> ZhviNowcastResult:
    """Live nowcast using fresh Cleveland + Zillow (ZORI + ZHVI) scrapes."""
    panel = fetch_panel()
    ...
    try:
        feats.update(
            _zhvi_features_for_month(zhvi_df, zori_df, target_month_end, used_zhvi)
        )
    except Exception:
        pass
