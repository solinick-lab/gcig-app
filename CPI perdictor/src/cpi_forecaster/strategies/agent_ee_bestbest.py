"""Agent EE: best-of-best with time-aware per-horizon model selection.

Round 4 strategy. The previous stacking attempt (champion_v3) failed
because tiny OOF training sets caused the meta-learner to overfit. This
strategy sidesteps that failure mode entirely: instead of LEARNING how
to combine base predictions, we just SELECT the best base learner per
horizon based on recent rolling-1-step-CV performance.
...
For each horizon h:
  1. Score each learner's MAE on the last 12 months of training data
     using rolling-1-step-CV (each month: train on history, predict
     next month, compare to actual).
...
Different horizons can pick different learners — that's the whole point.

Self-contained: does NOT import from sibling agent_*.py files.
"""
...
class BestOfBestStrategy(ForecastStrategy):
    """Per-horizon model selection over three diverse base learners."""

    name = "agent_ee_bestbest"
    ...
    def _predict_one_horizon(
        ...
        if len(df) < _MIN_TRAIN_ROWS:
            yhat = _last_observed_mom(y_full)
            sd = max(_empirical_mom_std(y_full), _RESID_FLOOR)
            return yhat - _Z80 * sd, yhat, yhat + _Z80 * sd
