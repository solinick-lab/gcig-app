"""PCA-reduced + quantile-regression CPI nowcaster (Agent UUU).

The pure-quantile nowcaster (`nowcast_quantile.py`) shoves ~30 daily
features into three GBR fits. Many of those features (oil ETFs, FX
crosses, equity sectors, treasury yields) are highly co-moving — a lot
of correlated noise relative to the signal that actually loads on
current-month CPI MoM.

Hypothesis: the daily-features signal lives in a low-dim subspace.
Strip it down to ~7 latent factors with PCA before fitting quantiles,
and the GBR sees a cleaner target manifold with less feature-noise
overfitting per cut.

CRITICAL DESIGN CHOICE: the lag features (cpi_mom_lag1, cpi_mom_lag2,
cpi_yoy_lag1) are NOT pushed through PCA. They carry strong direct,
near-linear signal that PCA — fit to maximize joint variance across all
features — would dilute against high-variance daily noise. We split the
feature matrix into "daily" (PCA-eligible) and "lag/calendar"
(passthrough), reduce only the daily block, then concatenate.

Pipeline:
    daily_X --> StandardScaler --> PCA(7 components) ---|
                                                        |--> [PCA(7) | lags | calendar] --> GBR(q=0.1, 0.5, 0.9)
    lag_X + calendar_X ----------------------------------|

Sort the (q0.1, q0.5, q0.9) triple per-prediction to enforce
monotonicity (separately-fit quantile regressors can cross). Clip MoM
to [-1.5, 2.5] before chaining to YoY. Each cut wrapped in try/except
so a single bad fit doesn't poison the 24-month walk-forward.

Standard interface — `backtest_pca_quantile_nowcast` and
`run_pca_quantile_nowcast` mirror the shapes used by the rest of the
nowcasting suite. <30s/cut budget on a quiet machine.
"""
...
# Columns that DO NOT get PCA-reduced — they pass through and are
# concatenated after the PCA features. These carry strong direct signal
# that PCA would otherwise dilute.
_PASSTHROUGH_COLS = (
    "cpi_mom_lag1",
    "cpi_mom_lag2",
    "cpi_yoy_lag1",
...
@dataclass
class PcaQuantileNowcastResult:
    as_of: pd.Timestamp
    target_month: str
    pred_mom: float
    pred_yoy: float
...
@dataclass
class PcaQuantileNowcastModel:
    scaler: StandardScaler
    pca: PCA
    models: dict[float, GradientBoostingRegressor]
    daily_cols: list[str]      # cols fed through PCA
    ...
    def _transform(self, x: pd.Series) -> np.ndarray:
        ...
        daily_vals = (
            x.reindex(self.daily_cols).fillna(0.0).values.reshape(1, -1).astype(float)
        )
        passthrough_vals = (
            x.reindex(self.passthrough_cols).fillna(0.0).values.reshape(1, -1).astype(float)
        )
...
def fit_pca_quantile_nowcast_model(
    ...
    # Cap n_components at min(n_features, n_samples - 1).
    ...
    return PcaQuantileNowcastModel(
        scaler=scaler,
        pca=pca,
        models=models,
...
def _build_inference_features(
    ...
    """Construct the feature row for one cut, simulating as-of `as_of_day`."""
...
def _mom_to_yoy(
    ...
    """Chain MoM log-% to YoY using last_cpi * exp(mom/100) over CPI[T-12]."""
...
def run_pca_quantile_nowcast(
    as_of_day: int = DEFAULT_AS_OF_DAY,
    n_components: int = _PCA_COMPONENTS,
) -> PcaQuantileNowcastResult:
    """Pull live panels, fit PCA+quantile model, produce current-month forecast."""
    panel = fetch_panel()
    daily_panel = get_daily_panel()
    daily_frame = build_daily_frame(daily_panel)

    model = fit_pca_quantile_nowcast_model(
        panel, daily_frame, as_of_day=as_of_day, n_components=n_components,
    )
    ...
    return PcaQuantileNowcastResult(
        as_of=as_of,
...
def backtest_pca_quantile_nowcast(
    panel: pd.DataFrame,
    daily_frame: dict[str, pd.Series],
    ...
    """Walk-forward backtest of the PCA+quantile nowcaster.

    Per cut t in the trailing `window_months`:
      - rebuild scaler + PCA + 3 quantile GBRs on data strictly before t
      - predict t's MoM (median of sorted quantile triple)
      - clip to [-1.5, 2.5], chain to YoY against actual CPI[t-12]

    Failed cuts are skipped, not raised — keeps the window honest if a
    single fit blows up. Output shape mirrors `nowcast.backtest_nowcast`.
    """
    cpi = panel[TARGET.fred_id].dropna()
    y_mom = build_target(panel).dropna()
